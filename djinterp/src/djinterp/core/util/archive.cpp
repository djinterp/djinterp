...
*
* path:      /src/cpp/archive.cpp
* link(s):   TBA
* author(s): Samuel 'teer' Neal-Blim                          date: 2026.05.23
*******************************************************************************/

#include "../../inc/cpp/archive.hpp"
...
#include <cstdlib>
    ...
    #include <zlib.h>   // crc32 for the zip central directory
#endif
...
// D_CFG_ARCHIVE_ALLOW_TOOL_SHELLOUT
//   configuration: permit the archive layer to spawn an external archiver
// (e.g. the proprietary `rar` tool, or `7z`) when no in-process backend can
// satisfy a request. Defaults to 1. Set to 0 to forbid process spawning
// entirely (e.g. in a sandbox), in which case those paths return
// status_unavailable.
#ifndef D_CFG_ARCHIVE_ALLOW_TOOL_SHELLOUT
    #define D_CFG_ARCHIVE_ALLOW_TOOL_SHELLOUT 1
#endif

// platform split for temp-dir / process helpers used by the tool shell-out
...
static void
put_u16_le(byte_buffer& _b, unsigned int _v)
{
...
static void
put_u32_le(byte_buffer& _b, unsigned long _v)
{
    _b.push_back((char)(unsigned char)(_v & 0xFFu));
    _b.push_back((char)(unsigned char)((_v >> 8)  & 0xFFu));
...
static unsigned int
read_u16_le(const char* _p, std::size_t _at)
{
...
read_u32_le(const char* _p, std::size_t _at)
{
    const unsigned char* u = (const unsigned char*)_p + _at;
...
static void
tar_octal(char* _field, std::size_t _width, unsigned long _val)
{
    std::size_t i;
...
static void
tar_write_entry(byte_buffer& _out, const entry& _e)
{
    char          header[512];
    unsigned long sum;
    std::size_t   i;
...
static status
tar_create(const entry* _items, std::size_t _count, byte_buffer& _out)
{
    std::size_t i;
...
static unsigned long
tar_parse_octal(const char* _p, std::size_t _width)
{
    unsigned long v = 0;
    std::size_t   i;
...
static status
tar_extract(const char* _in, std::size_t _n, entry_list& _out)
{
    std::size_t pos = 0;
           ...
           byte_buffer&             _out)
{
    ...
    for (i = 0; i < _count; ++i)
    {
        ...
        else
        {
            ...
            if (_opt.store_only || !D_INTERNAL_ZIP_HAVE_DEFLATE)
            {
                method  = 0;
                payload = e.data;
            }
...
// _unsupported.
static status
zip_extract(const char* _in, std::size_t _n, entry_list& _out)
{
    std::size_t pos = 0;
...
static status
gz_extract(const char* _in, std::size_t _n, entry_list& _out)
{
    entry  e;
    status s;
...
static status
tar_gz_create(const entry*            _items,
              std::size_t              _count,
              const archive_options&   _opt,
              byte_buffer&             _out)
{
...
static status
tar_gz_extract(const char* _in, std::size_t _n, entry_list& _out)
{
    byte_buffer tarball;
    status      s;

    s = decompress_buffer(codec_id_gzip, _in, _n, tarball);
    if (s != status_ok)
    {
        return s;
    }

    return tar_extract(tarball.data(), tarball.size(), _out);
}
...
la_read_all(const char* _in, std::size_t _n, entry_list& _out)
{
    ...
    rc = archive_read_open_memory(a, (void*)_in, _n);
    if (rc != ARCHIVE_OK)
    {
        archive_read_free(a);
        return status_backend_error;
...
static status
la_write_7z(const entry*           _items,
            std::size_t             _count,
            const archive_options&  _opt,
            byte_buffer&            _out)
{
...
static std::string
internal_path_join(const std::string& _dir, const std::string& _leaf)
{
    std::string out = _dir;
...
static bool
internal_write_file(const std::string& _path, const byte_buffer& _data)
{
...
static bool
internal_read_file(const std::string& _path, byte_buffer& _out)
{
...
static std::string
internal_shell_quote(const std::string& _s)
{
    std::string out;
...
static void
internal_remove_tree(const std::string& _dir)
{
    std::string cmd;
...
static status
rar_create_via_tool(const entry*  _items,
                    std::size_t    _count,
                    byte_buffer&   _out)
{
...
#if !D_ENV_ARCHIVE_HAVE_LIBARCHIVE
static status
archive_extract_via_tool(const char*  _in,
                        std::size_t    _n,
                        const char*    _tool,
                        entry_list&    _out)
{
...
static status
sevenzip_create(const entry*            _items,
                std::size_t              _count,
                const archive_options&   _opt,
                byte_buffer&             _out)
{
...
static status
sevenzip_extract(const char* _in, std::size_t _n, entry_list& _out)
{
#if D_ENV_ARCHIVE_HAVE_LIBARCHIVE
...
bool
format_can_write(format_id _fmt)
{
    switch (_fmt)
    {
...
bool
format_can_read(format_id _fmt)
{
    switch (_fmt)
    {
...
status
archive_extract(format_id     _fmt,
                const char*    _in,
                std::size_t    _n,
                entry_list&    _out)
{
