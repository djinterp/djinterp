 ...
 *****************************************************************************/
 ...
 /*
 D_TEST_OPTIONS_KEY_INVALID
   Pseudo-enum value returned by d_test_options_key_from_string() when a key
 ...
 */
#ifndef D_TEST_OPTIONS_KEY_INVALID
    #define D_TEST_OPTIONS_KEY_INVALID ((enum DTestConfigKey)UINT32_MAX)
#endif
  ...
  */
struct d_test_options_override_row
{
...
/*
d_internal_test_options_has_override
  Checks if config has an override value for the given flag.
*/
static bool
d_internal_test_options_has_override
(
    const struct d_test_options* _config,
    uint32_t                    _flag
)
{
...
/*
d_internal_test_options_get_override_ptr
...
/*
d_internal_test_options_set_override
  Sets an override value for the given flag.
*/
static bool
d_internal_test_options_set_override
(
    struct d_test_options* _config,
    uint32_t              _flag,
    void*                 _value
)
{
...
d_internal_test_options_get_value
(
    const struct d_test_options* _config,
    uint32_t                    _key
)
{
    ...
    if (_key == D_TEST_OPTIONS_MESSAGE_FLAGS)
    {
        if (_config)
        {
    ...
    // check for override
    if (d_internal_test_options_has_override(_config, _key))
    {
        override_ptr = d_internal_test_options_get_override_ptr(_config, _key);
...
)
{
    ...
    config = (struct d_test_options*)calloc(1, sizeof(*config));
...
struct d_test_options*
d_test_options_new_preset
(
    uint32_t _preset
)
{
    return d_test_options_new(_preset);
}

struct d_test_options*
    d_test_options_new_copy
    (
        const struct d_test_options* _other
    )
{
    ...
    copy = (struct d_test_options*)calloc(1, sizeof(*copy));
...
)
{
    ...
    v = d_internal_test_options_get_value(_config, _key);

    return v.b;
}

size_t
d_test_options_get_size_t
(
    const struct d_test_options* _config,
    uint32_t                    _key
)
...
)
{
    ...
    return d_internal_test_options_set_override(_config, _key, (void*)(uintptr_t)_value);
}

bool
d_test_options_set_size_t
(
    struct d_test_options* _config,
    uint32_t              _key,
    size_t                _value
...
)
{
    ...
    if ( (!schema_row) || 
         (schema_row->value_type != D_TYPE_INFO_INT32) )
    {
        return false;
    }

    return d_internal_test_options_set_override(_config, _key, (void*)(intptr_t)_value);
}

bool
d_test_options_set_uint32
(
    struct d_test_options* _config,
    uint32_t              _key,
    uint32_t              _value
...
)
{
    ...
    return d_internal_test_options_set_override(_config, _key, (void*)_value);
}

bool
d_test_options_set_ptr
(
    struct d_test_options* _config,
    uint32_t              _key,
    void*                 _value
)
{
    ...
    return d_internal_test_options_set_override(_config, _key, _value);
}
...
enum DTestConfigKey
d_test_options_key_from_string
(
    const char* _key
)
{
    ...
    if (!_key)
    {
        return D_TEST_OPTIONS_KEY_INVALID;
    }
