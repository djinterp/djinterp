...
// These are the only function pointers in the subframework that may be
// IMPLEMENTED in one language and INVOKED from the other, so their parameter
// and return widths are an ABI, not a convenience.  `bool` is the sharp case:
...
bool        d_doc_render_algebra_is_valid(
                const struct d_doc_render_algebra* _algebra);
bool        d_doc_render_algebra_reads(
                const struct d_doc_render_algebra* _algebra,
                const char*                        _key);
