...
*   Targets a tentative C99 language floor and supplies the common facilities
* assumed by other modules: cross-platform scalar types, the qualifier kit,
...
1.  GLOBAL CONSTANTS & KEYWORDS
    ...
    2.  Framework keywords
         1.  D_KEYWORD_BINARY
         2.  D_KEYWORD_CLI
         3.  D_KEYWORD_CONCURRENT
         4.  D_KEYWORD_CONTAINER
         ...
         13. D_KEYWORD_SYNC
         14. D_KEYWORD_TEST
         15. D_KEYWORD_TESTING
         16. D_KEYWORD_TEXT
         17. D_KEYWORD_UI
         18. D_KEYWORD_WARNING
2.  FUNDAMENTAL TYPES
    -----------------
...
// 1.1.1
// D_FRAMEWORK_NAME
//   constant: the framework's name as a bare token; resolves to `djinterp`.
#define D_FRAMEWORK_NAME             djinterp

// 1.2    Framework keywords
//------------------------------------------------------------------------------
// 1.2.1
// D_KEYWORD_BINARY
...
// Used for code involving operations that may proceed independently or overlap
// in execution.
...
// Used to specify that a unit of code pertains to containers: data structures
// that hold and manage collections of elements.
...
// Used to specify that a portion of code refers to file systems; namely
// dealing with files, directories, and reading/writing to and from files.
...
// 1.2.12
// D_KEYWORD_OPTION
//   keyword: resolves to `option`.
// Used for code pertaining to the options subframework, where key-value pairs
// correspond to configuration options, state variables in a registry, and
// more.
#define D_KEYWORD_OPTION             option

// 1.2.13
// D_KEYWORD_SYNC
//   keyword: resolves to `sync`.
// Used for code involving synchronization, coordination, or ordered access
// between operations.
#define D_KEYWORD_SYNC               sync
...
// 1.2.15
// D_KEYWORD_TESTING
//   keyword: resolves to `testing`.
// Used to signify that a code segment pertains to unit testing.
...
// including (but not limited to) converting, encoding, and processing text.
...
// 2.3    Qualifier kit
//------------------------------------------------------------------------------
// storage and linkage qualifiers. The public spellings are D_STATIC, D_INLINE,
// D_STATIC_INLINE, D_EXTERN_C, and D_EXTERN_C_BEGIN / D_EXTERN_C_END, plus the
...
// taken). So D_INLINE carries `static` in C but not in C++/MSVC (which is why
// the compounds are composed from an inline specifier that never contains
...
// D_EXTERN_C_END
//   macro: closes the linkage block that D_EXTERN_C_BEGIN opens.
...
// composition) to avoid a double-`static` in C.
    ...
    // empty before C23, which has no constexpr.
    #ifndef D_CONSTEXPR
    ...
    // D_STATIC_CONSTEXPR
    //   qualifier: internal linkage plus D_CONSTEXPR.
    #ifndef D_STATIC_CONSTEXPR
        #define D_STATIC_CONSTEXPR          static D_CONSTEXPR
    #endif  // D_STATIC_CONSTEXPR

    // D_CONSTEXPR_INLINE
    //   qualifier: D_INLINE, which in standard C is a `static inline`.
    #ifndef D_CONSTEXPR_INLINE
        #define D_CONSTEXPR_INLINE          D_INLINE
    #endif  // D_CONSTEXPR_INLINE

    // D_STATIC_CONSTEXPR_INLINE
    //   qualifier: D_STATIC_INLINE, spelled directly to avoid a double
    // `static`.
...
// rather than a linkage one. So neither spelling adds linkage here:
// D_INLINE_VAR is empty and D_CONSTEXPR_INLINE_VAR reduces to D_CONSTEXPR. As
...
// C++17 spelling.
    ...
    //   qualifier: D_CONSTEXPR alone, since C has no inline variables.
...
typedef void (*fn_apply_context)(void* _element,
                                 void* _context);
...
typedef int (*fn_comparator)(const void* _a,
                             const void* _b);
...
typedef size_t (*fn_write)(char* const _buffer,
                           size_t      _size);
...
// 4.2.1
// d_index_convert_fast / d_index_convert_safe
/**
 * @brief Converts a signed index to a zero-based offset without validating it.
 *
 * @param[in] _index  the index to convert.
 * @param[in] _count  the number of elements available.
 * @pre    `d_index_is_valid(_index, _count)` is `true`.
 * @return the zero-based offset that `_index` addresses; not meaningful when
 *         the precondition does not hold.
 */
size_t d_index_convert_fast(d_index _index,
                            size_t  _count);
/**
 * @brief Converts a signed index to a zero-based offset after validating it.
 *
 * @param[in]  _index        the index to convert.
 * @param[in]  _count        the number of elements available.
 * @param[out] _destination  receives the offset on success; left untouched on
 *                           failure.
 * @return `true` on success; `false` if `_destination` is `NULL` or `_index`
 *         is not valid for `_count`.
 */
...
// d_index_is_valid
/**
 * @brief Determines whether `_index` addresses one of `_count` elements.
 *
 * @note The limits are asymmetric: a non-negative index must be less than
 *       `_count`, while a negative index may have a magnitude equal to
 *       `_count`. For a count of 3, the valid indices are 0, 1, 2, -1, -2,
 *       and -3; a count of 0 has no valid index of either sign.
 *
 * @param[in] _index  the index to test.
 * @param[in] _count  the number of elements available.
 * @return `true` if `_index` addresses an element; `false` otherwise.
 */
...
// compile time. Evaluates `_index` more than once.
