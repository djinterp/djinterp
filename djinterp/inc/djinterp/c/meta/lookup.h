...
* parameterised on an offset and a width so the disagreements become
* arguments.
*
*   THE KEY IS TWO MEMBERS, NOT A DESCRIPTOR. kv.h carried a field
* descriptor and this view embedded one; both are gone. The view is
* unchanged at 40 bytes -- its padding absorbed the difference -- so it is
* the one consumer that neither gained nor lost by the removal.
*
*
...
struct d_lookup_view
{
    ...
    uint32_t          key_offset; // where the key sits within one record
    uint32_t          key_size;   // its width; 0 means the view is unusable
...
#define D_LOOKUP_FLAG_MASK                  ((uint16_t)0x000Fu)

// D_LOOKUP_FLAG_KEY_SIGNED
//   constant: the key holds a SIGNED integer, so a search driven by the view
// alone sign-extends it. The named forms -- d_lookup_find_signed and
// d_lookup_find_unsigned -- say it at the call site and ignore this flag; it
// exists for d_lookup_find_field, which has only the view to go on.
//   THIS IS WHERE THE DESCRIPTOR'S SIGNED BIT WENT. It was a flag on an
// embedded field descriptor; with that type gone it belongs where the view's
// other claims already live.
#define D_LOOKUP_FLAG_KEY_SIGNED            ((uint16_t)(1u << 3))
...
{                                                                              \
    NULL, 0u, 0u, 0u, 0u, D_LOOKUP_FLAG_NONE, 0u                               \
}
...
{                                                                              \
    ...
    D_KV_OFFSET(record, key_member, uint32_t),                                 \
    D_KV_WIDTH(record, key_member, uint32_t),                                  \
    (uint16_t)((flags) & D_LOOKUP_FLAG_MASK), 0u                               \
}

// D_LOOKUP_VIEW_OF_SIGNED
//   macro: the same, for a record whose key is a signed integer.
//   D_LOOKUP_FLAG_KEY_SIGNED IS FOLDED IN HERE, so a call site naming this
// macro cannot forget it. That flag is where the removed field descriptor's
// signed bit went; the named find functions still say it at the call site
// and ignore the flag entirely.
...
{                                                                              \
    ...
    D_KV_OFFSET(record, key_member, uint32_t),                                 \
    D_KV_WIDTH(record, key_member, uint32_t),                                  \
    (uint16_t)(((flags) | D_LOOKUP_FLAG_KEY_SIGNED) &                          \
               D_LOOKUP_FLAG_MASK), 0u                                        \
}
...
)
{
    ...
    if ( (!_records)                          ||
         ...
         (!d_kv_fits(_key_offset, _key_size, (uint32_t)_stride)) )
    {
        return view;
    }
    ...
    view.stride  = _stride;
    view.count      = _count;
    view.key_offset = _key_offset;
    view.key_size   = _key_size;
...
)
{
    ...
    return (const char*)d_kv_at_const(record, _view->key_offset);
}
...
)
{
    ...
    return d_kv_at_const(record, _view->key_offset);
}
...
)
{
    ...
    if ((_view->flags & D_LOOKUP_FLAG_KEY_SIGNED) != 0u)
    {
...
D_STATIC_ASSERT(offsetof(struct d_lookup_view, count) <
                    offsetof(struct d_lookup_view, key_offset),
                "d_lookup_view layout drift: count must precede key");
//   THE SLACK IS BOUNDED RATHER THAN FORBIDDEN, and the change is what
// dropping the field descriptor forced. The members used to sum to exactly
// sizeof, because a 12-byte descriptor happened to land the total on the
// alignment boundary; two uint32_t sum to four bytes less and the struct keeps
// its size, so four bytes of TAIL padding appear. That is the alignment doing
// its job, not drift. Interior padding would push the slack past one alignment
// unit, which is what this catches.
D_STATIC_ASSERT(( (sizeof(struct d_lookup_view) >=
                       ( sizeof(const void*) + sizeof(size_t) +
                         sizeof(size_t) + sizeof(uint32_t) +
                         sizeof(uint32_t) + sizeof(uint16_t) +
                         sizeof(uint16_t) ))                              &&
                  ((sizeof(struct d_lookup_view) -
                        ( sizeof(const void*) + sizeof(size_t) +
                          sizeof(size_t) + sizeof(uint32_t) +
                          sizeof(uint32_t) + sizeof(uint16_t) +
                          sizeof(uint16_t) )) < sizeof(const void*)) ),
                "d_lookup_view: a member width has introduced interior "
                "padding");
...
D_STATIC_ASSERT(( D_LOOKUP_FLAG_SORTED           |
                  D_LOOKUP_FLAG_KEY_IS_POINTER   |
                  D_LOOKUP_FLAG_CASE_INSENSITIVE |
                  D_LOOKUP_FLAG_KEY_SIGNED ) == D_LOOKUP_FLAG_MASK,
