...
*     merge_actual_only<_B,_D>   - B's full args, but B's actual<> arg
*                                  (if any) replaced by D's; if B has no
*                                  actual<>, D's is appended.
...
NS_INTERNAL
    ...
    // append _New.  Used by merge_actual_only and other "swap-this-tag"
    // operations.
...
// merge_actual_only
//   metafn: produce an option carrying _B's full args EXCEPT that _B's
// actual<> tag (if any) is replaced by _D's.  If _D has no actual<>,
// _B is returned unchanged.  If _B has no actual<>, _D's actual<> is
// appended.  Default, description, verifier, etc. always come from
// _B.  Useful for "value-only" delta sets.
template<typename _B,
         typename _D>
struct merge_actual_only
{
private:
    template<typename _BOpt, typename _DTag, bool _DHas>
    struct impl
    {
        using type = _BOpt;
    };

    template<typename _BOpt, typename _DTag>
    struct impl<_BOpt, _DTag, true>
    {
        using type =
            typename internal::option_swap_arg<_BOpt, is_actual, _DTag>::type;
    };

public:
    using type = typename impl<
        _B,
        option_actual_tag_t<_D>,
        option_has_actual_v<_D>>::type;
};
...
NS_INTERNAL
    ...
    private:
        ...
        // Lazy branch: only instantiate the on_both alias when we
        // genuinely have a B counterpart.
        template<bool _Has>
        struct pick
        {
        ...
        using produced = typename pick<in_b>::type;
...
// description, default_, verifier, etc.  Extension keys from B are
