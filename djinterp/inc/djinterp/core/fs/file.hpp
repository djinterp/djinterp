...
*
*   Umbrella for the C++ filesystem layer. Including this one header pulls
* in the whole layer -- the common base, every module, and the C++23 return
* surface -- so a caller who wants "all of djinterp's file support" writes a
* single include rather than tracking the dependency order by hand.
*
...
* in djinterp:: (never djinterp::fs::), the only nesting being
...
// file_status + status / symlink_status / file_size / exists / is_*
#include "file_stat.hpp"
#include "file_stream.hpp"      // file (RAII FILE* handle)
// directory (+ iterator) + create_directory / remove_directory
...
// C++23 std::expected overloads (inert below C++23)
#include "file_expected.hpp"
