...
private:
    ...
    write_pdf_report()
    {
        ...
        //    The template-based (*_doc) entry points build the report as
        //    document templates (title_page + document_table + colored check
        //    lines) and need no styled-op layout; see test_render_pdf.hpp
        //    Section V.
        ...
        if (split(m_opts) == test_output_split::per_module)
        {
            ...
            for (mi = 0; mi < m_report.modules.size(); ++mi)
            {
                ...
                pending_doc d;
                d.bytes = render_module_pdf_bytes_doc(fm);
        ...
        {
            pending_doc d;
            d.bytes = render_report_pdf_bytes_doc(m_report);
