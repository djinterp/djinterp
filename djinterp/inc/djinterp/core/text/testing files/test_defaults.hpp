...
// ---- swappable report / PDF sink (test seam) -------------------------------
//   Production (default) pulls the real report_builder - the report / PDF view
// the run_module / run_suite bridge drives (formerly reached via
// test_spec_runner.hpp).  That include is relative, so it resolves against THIS
// file's directory first and cannot be shadowed by -I; to observe drive_report
// activity a test build defines DTEST_DEFAULTS_USE_REPORT_DOUBLE and puts a
// basename drop-in "test_report_runner.hpp" on the include path (e.g.
// -I .../test_doubles).  The basename is not present next to this header, so it
// falls through to the -I double, whose module / open_unit / check / close_unit
// record to a process log while the tree + exit codes stay real.
#ifdef DTEST_DEFAULTS_USE_REPORT_DOUBLE
#  include "test_report_runner.hpp"           // instrumented drop-in (-I .../test_doubles)
#else
#  include "./output/test_report_runner.hpp"  // real report_builder / PDF view
#endif
