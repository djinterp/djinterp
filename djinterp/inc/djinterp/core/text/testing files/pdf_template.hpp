/******************************************************************************
* djinterp [text]                                             pdf_template.hpp
*
...
*
*
* path:      /inc/djinterp/core/text/pdf/pdf_template.hpp
* link(s):   TBA
* author(s): Samuel 'teer' Neal-Blim                       created: 2026.05.22
******************************************************************************/

#ifndef DJINTERP_TEXT_PDF_TEMPLATE_
#define DJINTERP_TEXT_PDF_TEMPLATE_ 1
...
public:
    ...
    // =================================================================
    //  d. binding surface (key / function / section resolution)
    // =================================================================

    // text_template was rewritten into an immutable, lookup-driven
    // engine that stores no bindings of its own, so the binding store
    // lives here now.  bind* accumulate key -> value, key -> resolver,
    // and section flags; emit_text assembles a lookup over them and
    // hands it to a per-element text_template at render time.  Missing
    // keys resolve to empty, matching text_template's lenient contract.
    //
    //   NOTE: text_template no longer supports block sections
    // ({#name}...{/name}); bind_section stores a flag that resolves the
    // bare key to itself (true) or empty (false), NOT a conditional
    // block.  If block sections are needed, they must be reintroduced
    // at this layer.

    // resolver_type
    //   the stored shape of a function binding: a nullary producer of
    // the value to substitute for its key.
    ...
    {
        m_values.emplace_back(_key, _value);
    ...
    {
        m_sections.emplace_back(_key, _value);
...
private:
    ...
    ) const
    {
        ...
        // parse this element's format once and render it against the
        // bound values / resolvers / sections; missing keys -> empty.
        text_template<> tmpl(_e.format);
        std::string resolved = tmpl.render(
            [this](std::string_view _key_view) -> std::string
            {
                const std::string _key(_key_view);

                for (const std::pair<std::string, std::string>& _kv
                         : m_values)
                {
                    if (_kv.first == _key)
                    {
                ...
                for (const std::pair<std::string, resolver_type>& _kf
                         : m_resolvers)
                {
                    if (_kf.first == _key)
                    {
                ...
                for (const std::pair<std::string, bool>& _ks
                         : m_sections)
                {
                    if (_ks.first == _key)
                    {
    ...
    std::vector<std::pair<std::string, resolver_type> > m_resolvers;
    std::vector<std::pair<std::string, bool> >          m_sections;
...
#endif  // DJINTERP_TEXT_PDF_TEMPLATE_
