...
// djinterp shared infrastructure. env_mysql_common.h is responsible for
// including the MariaDB/MySQL C client header and the MariaDB C++ connector
// header when D_CFG_ENV_USING_MARIADB=1, and for #error-ing if either
// cannot be located. After this include, D_ENV_MARIADB_HEADER_INCLUDED is
// either 1 (header in scope; detection below is safe) or 0 (no MariaDB
// symbols are referenced in this file).
