...
// cell_format
//   struct: the PER-CELL overrides a table may carry on top of its row style -
// a background fill, a text color, an alignment, and a font.  Each is opt-in
// (the has_* flag), so an untouched cell_format changes nothing and the default
// table path is byte-identical to what it was before this existed.
//
//   WHY IT IS HERE.  A report table wants a zebra stripe, a tinted failure row,
// and colored pass/fail chips - per-cell fills and colors the single-style grid
// could not express, which is why such tables were painted by hand directly on
// pdf_document.  Expressing them here means one table engine serves the plain
// grid and the styled report alike.
struct cell_format
{
    bool           has_fill;
    pdf_color      fill;
    ...
    cell_format()
        : has_fill      (false),
          fill          (pdf_color::white()),
          has_text_color(false),
          text_color    (pdf_color::black()),
          has_align     (false),
          align         (pdf_text_align::left),
          has_font      (false),
          font          (pdf_base_font::helvetica, 10.0)
    {}
};

// cell_format_fn
//   type: (row, column) -> the overrides that cell carries.  The row index is
// the caller's own: the header, when present, is row 0 and data rows keep the
// index they had in the supplied grid, so a zebra rule reads as plainly as
// `(row % 2) == 0`.  A null function means "no overrides anywhere".
using cell_format_fn = std::function<cell_format(std::size_t /*_row*/,
                                                 std::size_t /*_col*/)>;
...
// styles, grid line color/width, whether the first supplied row is a
// header, and an optional per-cell format callback.
struct table_options
{
    ...
    // cells
    //   optional per-cell overrides; null (the default) keeps the plain grid.
    cell_format_fn        cells;
...
public:
    ...
    // band
    //   a full-content-width filled strip with text flowed inside it -- the
    // flow-relative counterpart of text_box, which places at an absolute rect
    // and only strokes.  A section header, a suite band, a callout: anything
    // that wants a coloured ground behind flowing text.
    //
    //   The fill must be painted BEFORE the text, so the height is measured
    // first (wrap the text to the inner width, count the lines) rather than
    // discovered by flowing.  That is why this cannot simply be paragraph()
    // with a colour: the cursor only knows where the text ended after it has
    // already been drawn.
    void
    band(
        const std::string&  _text,
    ...
    )
    {
        ...
        const pdf_unit inner = content_width() - (2.0 * _padding);

        // a degenerate content box has nothing to band
        if (inner <= 0.0)
        {
            return;
        }
        ...
        const std::vector<std::string> lines =
            wrap_to_width(_style.font, _text, inner);

        const size_type line_count =
            (lines.empty() ? size_type(1) : lines.size());

        const pdf_unit height =
            (static_cast<pdf_unit>(line_count) * _style.leading) +
            (2.0 * _padding);

        // keep the whole band together: break before splitting it
        ensure_room(height);

        const pdf_unit top = m_cursor_y;

        m_doc->rect(pdf_rect(m_margin_left, top - height,
                             content_width(), height),
                    pdf_paint::filled(_fill));

        // flow the measured lines inside the padded box
        pdf_unit y = top - _padding - _style.font.size;

        for (size_type i = 0; i < lines.size(); ++i)
        {
            pdf_unit x = m_margin_left + _padding;

            // the same measured placement the table cells use
            if ( (_style.align == pdf_text_align::center) ||
                 (_style.align == pdf_text_align::right) )
            {
                const pdf_unit w = text_width(_style.font, lines[i]);

                x = (_style.align == pdf_text_align::center)
                        ? (m_margin_left + ((content_width() - w) * 0.5))
                        : (m_margin_left + content_width() - _padding - w);
            }

            m_doc->text(pdf_point(x, y), lines[i], _style.to_text_options());

            y -= _style.leading;
        }

        m_cursor_y = top - height - _style.space_after;
    ...
    )
    {
             ...
             (!_rows.empty()) )
        {
            ...
            draw_table_row(header, widths, _opts, true, size_type(0));
        }
        ...
        for (size_type r = start; r < _rows.size(); ++r)
        {
            ...
            draw_table_row(_rows[r], widths, _opts, false, r);
        }
...
private:
    ...
    )
    {
        ...
        // header background fill (the whole-row form; a per-cell fill below
        // paints over it, which is what lets a chip sit in a filled header)
        if ( (_is_header) &&
             (_opts.fill_header) )
        {
        ...
        // cells: optional fill, grid box, then aligned + clipped text
        for (size_type c = 0; c < _widths.size(); ++c)
        {
            pdf_unit   w      = _widths[c];
            cell_format fmt;

            // the caller's overrides for this cell, when it supplied any
            if (_opts.cells)
            {
                fmt = _opts.cells(_row_index, c);
            }

            // a per-cell fill goes down first, under the rule
            if (fmt.has_fill)
            {
                m_doc->rect(pdf_rect(x, bottom, w, _opts.row_height),
                            pdf_paint::filled(fmt.fill));
            }
            ...
            if (c < _cells.size())
            {
                canvas_style cell_style = style;

                // refine the row style by whichever overrides are present
                if (fmt.has_text_color) { cell_style.color = fmt.text_color; }
                if (fmt.has_font)       { cell_style.font  = fmt.font;       }
                if (fmt.has_align)      { cell_style.align = fmt.align;      }

                pdf_unit    inner = w - (2.0 * _opts.padding);
                std::string text  = truncate_ellipsis(
                    cell_style.font.family, cell_style.font.size,
                    _cells[c], inner);

                // horizontal placement within the cell box, measured
                pdf_unit tx = x + _opts.padding;

                if ( (cell_style.align == pdf_text_align::center) ||
                     (cell_style.align == pdf_text_align::right) )
                {
                    const pdf_unit tw = text_width(cell_style.font, text);

                    tx = (cell_style.align == pdf_text_align::center)
                             ? (x + ((w - tw) * 0.5))
                             : (x + w - _opts.padding - tw);

                    // never let a wide cell spill left of its own box
                    if (tx < (x + _opts.padding))
                    {
                        tx = x + _opts.padding;
                    }
                }
                ...
                pdf_unit ty = bottom +
                              ((_opts.row_height - cell_style.font.size) * 0.5);

                m_doc->text(pdf_point(tx, ty), text,
                            cell_style.to_text_options());
            }
