...
* to a sink.  Content production (state -> byte_blob, in binding_env /
...
*
*   DEFERRED PRODUCERS:
*   A bundle item holds a producer_type (std::function<byte_blob()>), not a
...
* std::function); producer.hpp's pull-based stream type is a different tool,
* for  sequences of values rather than a single deferred document.
*
*   THE WRITE ORCHESTRATOR:
*   write(bundle, cfg, sink) renders each item once, applies cfg's packaging
* policy, and hands the result(s) to the sink.  In `none` / `compress` mode
* each document is an independent (name, bytes) write; in `archive` mode every
...
* selectors are dispatched through the compress.hpp / archive.hpp internal
...
struct bundle_item
{
    ...
    using producer_type = std::function<byte_blob()>;
...
// compress.hpp's dispatch leaf.  Returns the facade status.
...
)
{
    ...
    for (_i = 0; _i < _total; ++_i)
    {
        std::string _name = internal::resolve_name(_cfg,
                                                   ...
                                                   _i,
                                                   _total);
        byte_blob _bytes = _items[_i].produce();   // deferred render (once)
        ...
        if (_cfg.pack == pack_mode::compress)
        {
            ...
            _bytes = static_cast<byte_blob&&>(_packed);
        }
