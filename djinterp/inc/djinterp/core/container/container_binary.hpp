    ...
    std::uint64_t _payload) noexcept
{
    ...
    if constexpr (
        traits::has_type_info_integration_v<clean_type>)
    {
        h.flags |=
            binary::binary_flags::has_type_info;
    }

    if constexpr (traits::has_trivially_copyable_elements_v<clean_type>)
    {
        h.flags |= binary::binary_flags::fixed_elements;

        h.element_size = static_cast<std::uint32_t>(sizeof(typename clean_type::value_type));

        h.type_info = static_cast<d_type_info64>(binary::type_info_for_v<typename clean_type::value_type>);
    }
