...
// mysql.hpp would be included here once it exists; for now we
// forward-declare mysql_connection so the template is well-formed.
// Translation units that instantiate mysql_table must include
// mysql.hpp themselves.
...
NS_DB
    ...
    class mysql_table
        : public mysql_common::mysql_common_table<
              mysql_connection,
              value,
              _Config>
    {
    private:
        using base_type = mysql_common::mysql_common_table<
            mysql_connection,
            value,
            _Config>;
