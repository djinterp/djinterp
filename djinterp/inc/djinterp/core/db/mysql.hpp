...
// the shared mysql_common::mysql_connect_config.
struct mysql_connect_config
{
    mysql_common::mysql_connect_config  common;
...
class mysql_connection
    : public mysql_common::mysql_common_connection<
          mysql_connection,
          database_type::mysql>
{
private:
    using base_type = mysql_common::mysql_common_connection<
...
public:
    ...
    // -----------------------------------------------------------------
    // _impl methods (defined in mysql.cpp)
    // -----------------------------------------------------------------
    ...
    auto        execute_query_impl(const std::string& _query)
                    -> std::unique_ptr<
                        result_set<struct mysql_result_set_impl>>;
    ...
    auto        prepare_impl(const std::string& _query)
                    -> std::unique_ptr<
                        statement<struct mysql_statement_impl>>;
    ...
    // MySQL-family common _impl methods
    void          set_charset_impl(const std::string& _charset);
    std::string   get_charset_impl() const;
    int           next_result_impl();
    bool          more_results_impl() const;
    std::string   escape_string_impl(
                      const std::string& _input) const;
    void          select_db_impl(const std::string& _database);
    void          change_user_impl(const std::string& _user,
    ...
    void          set_autocommit_impl(bool _enabled);
    void          set_option_impl(int         _option,
                                  const void* _value);

    // schema introspection _impl
