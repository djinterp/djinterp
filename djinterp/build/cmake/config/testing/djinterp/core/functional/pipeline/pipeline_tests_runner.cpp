/******************************************************************************
* djinterp [test]                                      pipeline_test_runner.cpp
*
*   Standalone entry point for the pipeline.hpp unit-test binary, relocated
* into the per-module subdirectory .../functional/pipeline/ to match the event
* subsystem's layout (one runner per module, under its own folder).
*
*   pipeline_tests.hpp is a DTest run_session suite: its sections are
* `void(test_handler&)` functions scheduled by pipeline_module_run_all against a
* test_runner_ctx, and pipeline_module_info carries the module's identity. Both
* are declared in the header and DEFINED in the suite's wiring translation unit
* (pipeline_tests_runner.cpp); this file only drives them. The
* session_descriptor is derived from pipeline_module_info so the module's
...
*
*   SINGLE ENTRY POINT:
*   pipeline_tests_runner.cpp (the wiring TU) still carries an inline main() for
...
* pulled in transitively via pipeline_tests.hpp.
*
* path:      /build/cmake/config/testing/djinterp/core/functional/pipeline/pipeline_test_runner.cpp
* link(s):   TBA
* author(s): Samuel 'teer' Neal-Blim                       created: 2026.07.06
******************************************************************************/

// djinterp
#include "pipeline_tests.hpp"             // resolved via the test include path
...
main()
{
    ...
    // description_long - see test_runner.hpp section III), so pipeline's
    // identity is stated once, in pipeline_module_info.
