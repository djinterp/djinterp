/******************************************************************************
* djinterp [test]                                        filter_test_runner.cpp
*
*   Enriched runner for the filter.hpp unit tests.  The suite describes itself:
* filter_tests.hpp, compiled with DTEST_SPEC_MODE, exposes filter_spec() - the
* suite's blocks as data, each unit test paired with a name and a one-line
* descriptor.  run_module (test_defaults.hpp) lowers that spec into the six-kind
* tree, describes it, and drives the report to the console and - as configured
* by a test_option_set - a PDF.  This file is only the entry point.
*
...
* document renders).  Each of the eight sections becomes one block; each
...
* run_all_filter_tests() are retained in the header for a bare pass/fail check,
* but the spec is the primary surface.
*
...
*     opts.set<test_option::output_file>("filter_tests.pdf");
...
* run_module / test_option_set surface is C++11+).  filter.hpp and its section
* tests are themselves C++11+ (named functors so the decltype-driven traits and
* filterable sections are well-formed pre-C++20); only this driver requires the
* framework floor.
*
* path:      /build/cmake/config/testing/djinterp/core/functional/filter/filter_test_runner.cpp
* link(s):   TBA
* author(s): Samuel 'teer' Neal-Blim                       created: 2026.07.06
******************************************************************************/

#define DTEST_SPEC_MODE                    // expose filter_spec(); omit fixtures
#include "filter_tests.hpp"                // resolved via the test include path
...
main()
{
    ...
    opts.set<dt::test_option::output_file>(
        std::string("filter_tests.pdf"));

    return dt::run_module(
        ::djinterp::testing::filter_spec(),
        "filter.hpp unit tests",
