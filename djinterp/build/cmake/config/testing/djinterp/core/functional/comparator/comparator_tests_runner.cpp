/******************************************************************************
* djinterp [test]                                     comparator_test_runner.cpp
*
*   Enriched runner for the comparator.hpp unit tests.  The suite describes
* itself: comparator_tests.hpp, compiled with DTEST_SPEC_MODE, exposes
* comparator_spec() - the suite's blocks as data, each unit test paired with a
...
* run_all_comparator_tests() are retained in the header for the C++98-subset
...
*     opts.set<test_option::output_file>("comparator_tests.pdf");
...
* run_module / test_option_set surface is C++11+).  comparator.hpp is itself
* dual-standard: its C++98 fallback path is exercised by compiling the section
* tests - which are written to the C++98 subset - under C++98 and driving them
* through the retained run_all_comparator_tests() aggregate; this run_module
* driver covers the C++11+ primary path.
*
* path:      /build/cmake/config/testing/djinterp/core/functional/comparator/comparator_test_runner.cpp
* link(s):   TBA
* author(s): Samuel 'teer' Neal-Blim                       created: 2026.07.06
******************************************************************************/

#define DTEST_SPEC_MODE                    // expose comparator_spec(); omit fixtures
#include "comparator_tests.hpp"            // resolved via the test include path
...
main()
{
    ...
    opts.set<dt::test_option::output_file>(
        std::string("comparator_tests.pdf"));

    return dt::run_module(
        ::djinterp::testing::comparator_spec(),
        "comparator.hpp unit tests",
