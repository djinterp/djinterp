...
*
*   Unit tests for the core detection-macro family, now DECLARED in
* trait_detect.hpp (re-exported through Section 0.3 of type_traits.hpp):
*     - D_VOID_T
*     - D_TYPE_TRAIT_VALUE_BOOL     (was D_TRAIT_VALUE_BOOL)
*     - D_TYPE_TRAIT_TYPE_ALIAS     (was D_TRAIT_TYPE_ALIAS)
*     - D_TYPE_TRAIT_TRUE_AS        (was D_TRAIT_IS_DETECTED_AS)
*     - D_TYPE_TRAIT_TRUE           (was D_TRAIT_IS_DETECTED)
*     - D_TYPE_TRAIT_TRUE_FROM      (was D_TRAIT_IS_DETECTED_FROM)
*
...
*   - D_TYPE_TRAIT_TRUE_FROM: tested via a trait that succeeds based
*     on detection AND inherits from std::is_integral<_Type> — so the
*     final answer depends on BOTH the probe AND the delegate trait.
*   - D_TYPE_TRAIT_VALUE_BOOL: tested implicitly through every macro that
*     emits a `_v` alias (every IS_DETECTED variant).
*   - D_TYPE_TRAIT_TYPE_ALIAS: applied to a trait that exposes a `::type`
...
// same type. Used by the multi-expression variant of D_TYPE_TRAIT_TRUE
...
static_assert(macro_trait_count_7<without_value_type>::value == false,
              "D_TYPE_TRAIT_TRUE_AS failure: ::value is false");
static_assert(std::is_base_of<std::false_type,
                              macro_trait_count_7<without_value_type>>::value,
              "D_TYPE_TRAIT_TRUE_AS failure: base is std::false_type");


// =========================================================================
// IV.  D_TYPE_TRAIT_TRUE_FROM  (compile-time)
// =========================================================================
...
D_TYPE_TRAIT_TRUE_FROM(macro_trait_has_vt_and_integral,
    typename _Type::value_type,
    std::is_integral)
...
static_assert(macro_trait_has_vt_and_integral<int>::value == false,
              "D_TYPE_TRAIT_TRUE_FROM: int has no value_type -> false");
...
static_assert(macro_trait_has_vt_and_integral<with_value_type_int>::value == false,
              "D_TYPE_TRAIT_TRUE_FROM: probe ok, delegate is_integral false -> false");

#if D_ENV_CPP_FEATURE_LANG_VARIABLE_TEMPLATES
    static_assert(macro_trait_has_vt_and_integral_v<int> == false,
                  "D_TYPE_TRAIT_TRUE_FROM _v alias (false)");
#endif


// =========================================================================
// V.   D_TYPE_TRAIT_TYPE_ALIAS  (compile-time)
// =========================================================================
...
// use std::add_const directly — but D_TYPE_TRAIT_TYPE_ALIAS expects a NAME,
// so we wrap it.
...
D_TYPE_TRAIT_TYPE_ALIAS(macro_type_alias_subject)

static_assert(std::is_same<macro_type_alias_subject_t<int>, const int>::value,
              "D_TYPE_TRAIT_TYPE_ALIAS: alias resolves to wrapped ::type");
static_assert(std::is_same<macro_type_alias_subject_t<float>, const float>::value,
              "D_TYPE_TRAIT_TYPE_ALIAS: alias works for arbitrary types");


// =========================================================================
// VI.  D_TYPE_TRAIT_VALUE_BOOL  (compile-time)
// =========================================================================
...
D_TYPE_TRAIT_VALUE_BOOL(macro_value_bool_subject)

#if D_ENV_CPP_FEATURE_LANG_VARIABLE_TEMPLATES
    static_assert(macro_value_bool_subject_v<int*>    == true,
                  "D_TYPE_TRAIT_VALUE_BOOL emits _v alias (true case)");
    static_assert(macro_value_bool_subject_v<int>     == false,
                  "D_TYPE_TRAIT_VALUE_BOOL emits _v alias (false case)");
    static_assert(macro_value_bool_subject_v<int**>   == true,
                  "D_TYPE_TRAIT_VALUE_BOOL emits _v alias (pointer-to-pointer)");
#endif
...
)
{
    ...
    // ---- D_TYPE_TRAIT_TRUE (single expr) ----
    record_assertion(_test_handler, 
        macro_trait_value_type<with_value_type_int>::value == true,
        "D_TYPE_TRAIT_TRUE single: with value_type -> true");
    record_assertion(_test_handler, 
        macro_trait_value_type<without_value_type>::value == false,
        "D_TYPE_TRAIT_TRUE single: without value_type -> false");
    record_assertion(_test_handler, 
        macro_trait_value_type<int>::value == false,
        "D_TYPE_TRAIT_TRUE single: builtin int -> false");

    // ---- D_TYPE_TRAIT_TRUE (multi expr) ----
    record_assertion(_test_handler, 
        macro_trait_value_type_and_data<with_type_and_data>::value == true,
        "D_TYPE_TRAIT_TRUE multi: both probes succeed -> true");
    record_assertion(_test_handler, 
        macro_trait_value_type_and_data<with_type_only_no_data>::value == false,
        "D_TYPE_TRAIT_TRUE multi: second probe fails -> false");
    record_assertion(_test_handler, 
        macro_trait_value_type_and_data<without_value_type>::value == false,
        "D_TYPE_TRAIT_TRUE multi: first probe fails -> false");

    // ---- D_TYPE_TRAIT_TRUE_AS ----
    record_assertion(_test_handler, 
        macro_trait_count_7<with_value_type_int>::value == 7,
        "D_TYPE_TRAIT_TRUE_AS: success inherits supplied base (value=7)");
    record_assertion(_test_handler, 
        std::is_base_of<std::integral_constant<int, 7>,
                        macro_trait_count_7<with_value_type_int>>::value,
        "D_TYPE_TRAIT_TRUE_AS: success base type matches INHERIT_EXPR");
    record_assertion(_test_handler, 
    ...
    record_assertion(_test_handler, 
        ...
        "D_TYPE_TRAIT_TRUE_FROM: int (no value_type) -> false");
    record_assertion(_test_handler, 
        macro_trait_has_vt_and_integral<with_value_type_int>::value == false,
        "D_TYPE_TRAIT_TRUE_FROM: probe ok but delegate false -> false");

    // ---- D_TYPE_TRAIT_TYPE_ALIAS ----
    record_assertion(_test_handler, 
        std::is_same<macro_type_alias_subject_t<int>, const int>::value,
        "D_TYPE_TRAIT_TYPE_ALIAS: alias resolves to ::type (int -> const int)");
    record_assertion(_test_handler, 
        std::is_same<macro_type_alias_subject_t<float>, const float>::value,
        "D_TYPE_TRAIT_TYPE_ALIAS: alias resolves to ::type (float -> const float)");

    // ---- D_TYPE_TRAIT_VALUE_BOOL ----
#if D_ENV_CPP_FEATURE_LANG_VARIABLE_TEMPLATES
    record_assertion(_test_handler, 
        macro_value_bool_subject_v<int*> == true,
        "D_TYPE_TRAIT_VALUE_BOOL: _v alias (true case)");
    record_assertion(_test_handler, 
        macro_value_bool_subject_v<int> == false,
        "D_TYPE_TRAIT_VALUE_BOOL: _v alias (false case)");
#endif
