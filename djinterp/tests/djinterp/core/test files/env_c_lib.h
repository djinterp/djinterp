...
*   capabilities.  Despite the D_ENV_C_HAS_* naming, these flags describe the
*   C runtime that BOTH C and C++ translation units rely on, so the block is
*   gated on hosted-vs-freestanding (__STDC_HOSTED__), NOT on source language.
*
*   SCOPE:
...
*
*   Requires:  env.h  (for the D_ENV_LANG_*, D_ENV_OS_*, D_ENV_ARCH_*,
*              D_ENV_COMPILER_*, and D_ENV_IS_OS_POSIX_LIKE* macros used
*              throughout).  This header is an internal component of env.h
*              and is #included by it after all environment detection is
*              complete -- do NOT #include it directly.
*
*   NAMING CONVENTION:
*     D_ENV_C_HAS_[FEATURE_NAME] - 1 if available, 0 otherwise
*
* path:      /inc/c/core/env/c/env_c_lib.h
* link(s):   TBA
