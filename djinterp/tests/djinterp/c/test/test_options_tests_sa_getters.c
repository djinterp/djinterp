...
d_tests_sa_config_get_bool
  Tests the d_test_options_get_bool function.
  Tests the following:
  - Returns schema default when no override is set
  - Returns overridden value after set_bool
  - D_TEST_OPTIONS_ENABLED default is true
  - D_TEST_OPTIONS_SKIP default is false
*/
bool
d_tests_sa_config_get_bool
(
...
)
{
    ...
    result = d_assert_standalone(
        d_test_options_get_bool(config, D_TEST_OPTIONS_ENABLED) == true,
    ...
    result = d_assert_standalone(
        d_test_options_get_bool(config, D_TEST_OPTIONS_SKIP) == false,
        "get_bool_skip_default",
        "SKIP default should be false",
        _counter) && result;

    // test 3: after override, returns new value
    d_test_options_set_bool(config, D_TEST_OPTIONS_SKIP, true);

    result = d_assert_standalone(
        d_test_options_get_bool(config, D_TEST_OPTIONS_SKIP) == true,
        "get_bool_after_set",
...
)
{
    ...
    result = d_assert_standalone(
        d_test_options_get_size_t(config, D_TEST_OPTIONS_TIMEOUT_MS) ==
    ...
    result = d_assert_standalone(
        d_test_options_get_size_t(config, D_TEST_OPTIONS_MAX_FAILURES) ==
    ...
    result = d_assert_standalone(
        d_test_options_get_size_t(config, D_TEST_OPTIONS_INDENT_MAX_LEVEL) ==
    ...
    d_test_options_set_size_t(config, D_TEST_OPTIONS_TIMEOUT_MS, 5000);

    result = d_assert_standalone(
        d_test_options_get_size_t(config, D_TEST_OPTIONS_TIMEOUT_MS) == 5000,
...
)
{
    ...
    result = d_assert_standalone(
        d_test_options_get_int32(config, D_TEST_OPTIONS_PRIORITY) == 0,
        "get_int32_priority_default",
        "PRIORITY default should be 0",
        _counter) && result;

    // test 2: after override
    d_test_options_set_int32(config, D_TEST_OPTIONS_PRIORITY, -42);

    result = d_assert_standalone(
        d_test_options_get_int32(config, D_TEST_OPTIONS_PRIORITY) == -42,
        "get_int32_after_set",
    ...
    result = d_assert_standalone(
        d_test_options_get_int32(config, D_TEST_OPTIONS_ENABLED) == 0,
...
)
{
    ...
    result = d_assert_standalone(
        d_test_options_get_uint32(config, D_TEST_OPTIONS_MESSAGE_FLAGS) ==
            D_TEST_MODE_NORMAL,
    ...
    d_test_options_set_uint32(config,
...
d_tests_sa_config_get_string
  Tests the d_test_options_get_string function.
  Tests the following:
...
)
{
    ...
    indent = d_test_options_get_string(config, D_TEST_OPTIONS_INDENT_STR);
    ...
    // test 2: after override
    d_test_options_set_string(config, D_TEST_OPTIONS_INDENT_STR, "\t");
    indent = d_test_options_get_string(config, D_TEST_OPTIONS_INDENT_STR);
    ...
    result = d_assert_standalone(
        d_test_options_get_string(config, D_TEST_OPTIONS_ENABLED) == NULL,
...
)
{
    ...
    ptr = d_test_options_get_ptr(config, D_TEST_OPTIONS_INDENT_STR);
    ...
    d_test_options_set_ptr(config, D_TEST_OPTIONS_INDENT_STR, &dummy);
    ptr = d_test_options_get_ptr(config, D_TEST_OPTIONS_INDENT_STR);
...
)
{
    ...
    result = d_assert_standalone(
        d_test_options_get_bool(NULL, D_TEST_OPTIONS_ENABLED) == true,
    ...
    result = d_assert_standalone(
        d_test_options_get_bool(NULL, D_TEST_OPTIONS_SKIP) == false,
        "get_null_bool_skip",
    ...
    result = d_assert_standalone(
        d_test_options_get_size_t(NULL, D_TEST_OPTIONS_TIMEOUT_MS) ==
            D_TEST_DEFAULT_TIMEOUT,
    ...
    result = d_assert_standalone(
        d_test_options_get_int32(NULL, D_TEST_OPTIONS_PRIORITY) == 0,
    ...
    result = d_assert_standalone(
        d_test_options_get_string(NULL, D_TEST_OPTIONS_INDENT_STR) != NULL,
