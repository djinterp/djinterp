...
d_tests_sa_config_set_bool
  Tests the d_test_options_set_bool function.
  Tests the following:
...
)
{
    ...
    result = d_assert_standalone(
        d_test_options_set_bool(config, D_TEST_OPTIONS_SKIP, true) == true,
    ...
    // test 3: toggle back
    d_test_options_set_bool(config, D_TEST_OPTIONS_SKIP, false);
...
)
{
    ...
    result = d_assert_standalone(
        d_test_options_set_size_t(config,
    ...
    result = d_assert_standalone(
        d_test_options_get_size_t(config, D_TEST_OPTIONS_TIMEOUT_MS) == 2500,
    ...
    d_test_options_set_size_t(config, D_TEST_OPTIONS_MAX_FAILURES, 100);

    result = d_assert_standalone(
        d_test_options_get_size_t(config, D_TEST_OPTIONS_MAX_FAILURES) == 100,
...
)
{
    ...
    result = d_assert_standalone(
        d_test_options_set_int32(config, D_TEST_OPTIONS_PRIORITY, 10) == true,
        "set_int32_positive",
        "set_int32 should succeed for positive value",
        _counter) && result;

    result = d_assert_standalone(
        d_test_options_get_int32(config, D_TEST_OPTIONS_PRIORITY) == 10,
        "set_int32_positive_value",
        "PRIORITY should reflect positive set value",
        _counter) && result;

    // test 2: negative value
    d_test_options_set_int32(config, D_TEST_OPTIONS_PRIORITY, -99);

    result = d_assert_standalone(
        d_test_options_get_int32(config, D_TEST_OPTIONS_PRIORITY) == -99,
        "set_int32_negative",
        "PRIORITY should reflect negative set value",
        _counter) && result;

    // test 3: zero value
    d_test_options_set_int32(config, D_TEST_OPTIONS_PRIORITY, 0);
...
)
{
    ...
    result = d_assert_standalone(
        d_test_options_set_string(config,
...
)
{
    ...
    result = d_assert_standalone(
        d_test_options_get_ptr(config, D_TEST_OPTIONS_INDENT_STR) == &dummy,
        "set_ptr_value",
        "get_ptr should return the set pointer",
        _counter) && result;

    // test 2: set NULL pointer
    d_test_options_set_ptr(config, D_TEST_OPTIONS_INDENT_STR, NULL);

    result = d_assert_standalone(
        d_test_options_get_ptr(config, D_TEST_OPTIONS_INDENT_STR) == NULL,
        "set_ptr_null",
...
)
{
    ...
    result = d_assert_standalone(
        d_test_options_set_size_t(NULL, D_TEST_OPTIONS_TIMEOUT_MS, 1) == false,
        "set_null_size_t",
        "set_size_t(NULL) should return false",
        _counter) && result;

    result = d_assert_standalone(
        d_test_options_set_int32(NULL, D_TEST_OPTIONS_PRIORITY, 1) == false,
        "set_null_int32",
        "set_int32(NULL) should return false",
        _counter) && result;

    result = d_assert_standalone(
        d_test_options_set_uint32(NULL, D_TEST_OPTIONS_MESSAGE_FLAGS, 1) == false,
        "set_null_uint32",
        "set_uint32(NULL) should return false",
        _counter) && result;

    result = d_assert_standalone(
        d_test_options_set_string(NULL, D_TEST_OPTIONS_INDENT_STR, "x") == false,
        "set_null_string",
        "set_string(NULL) should return false",
        _counter) && result;

    result = d_assert_standalone(
        d_test_options_set_ptr(NULL, D_TEST_OPTIONS_INDENT_STR, NULL) == false,
        "set_null_ptr",
...
)
{
    ...
    result = d_assert_standalone(
        d_test_options_set_bool(config,
                               D_TEST_OPTIONS_TIMEOUT_MS,
                               true) == false,
    ...
    result = d_assert_standalone(
        d_test_options_set_int32(config,
                                D_TEST_OPTIONS_ENABLED,
                                42) == false,
