...
bool d_tests_sa_handler_abort_on_failure(struct d_test_counter* _test_info)
{
    ...
    { struct d_test_handler* h = d_test_handler_new(NULL);
      if (h) { if (!d_assert_standalone(h->abort_on_failure == false,
          "fresh abort=false", "Starts false", _test_info)) ok = false;
        d_test_handler_free(h); } }

    // Set via flag
    { struct d_test_handler* h = d_test_handler_new_full(NULL, 0, 0, D_TEST_HANDLER_FLAG_ABORT_ON_FAIL);
      if (h) { if (!d_assert_standalone(h->abort_on_failure == true,
          "flag sets abort", "Flag sets abort_on_failure", _test_info)) ok = false;
        d_test_handler_free(h); } }
    ...
    { struct d_test_handler* h = d_test_handler_new(NULL);
      if (h) { int i;
        ...
        if (b) {
          for (i = 0; i < 2; i++) helper_add_passing_test_to_block(b);
          ...
          if (!d_assert_standalone(
              (r.tests_run == 5 && r.tests_passed == 4 && r.tests_failed == 1),
              "disabled: all 5 run", "All 5 run when abort disabled", _test_info))
              ok = false;
          d_test_block_free(b); }
        d_test_handler_free(h); } }
    ...
    { struct d_test_handler* h = d_test_handler_new(NULL);
      if (h) { int i;
        ...
        if (b) {
          ...
          struct d_test_result r = d_test_handler_get_results(h);
          if (!d_assert_standalone(
              (r.tests_run == 3 && r.tests_passed == 2 && r.tests_failed == 1),
              "enabled: 3 of 5", "Abort stops at first failure", _test_info))
              ok = false;
          d_test_block_free(b); }
        d_test_handler_free(h); } }
    ...
    { struct d_test_handler* h = d_test_handler_new(NULL);
      if (h) { int i;
        ...
        struct d_test_block* b = d_test_block_new(NULL, 0);
        if (b) {
          for (i = 0; i < 5; i++) helper_add_passing_test_to_block(b);
          d_test_handler_run_block(h, b, NULL);
          struct d_test_result r = d_test_handler_get_results(h);
          if (!d_assert_standalone(r.tests_run == 5,
              "no failures: all run", "All 5 run when no failures", _test_info))
              ok = false;
          d_test_block_free(b); }
        d_test_handler_free(h); } }

    if (ok) { _test_info->tests_passed++; printf("%s[PASS] abort_on_failure\n", D_INDENT); }
    else printf("%s[FAIL] abort_on_failure\n", D_INDENT);
