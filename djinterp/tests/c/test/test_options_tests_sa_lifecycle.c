...
d_tests_sa_config_new
  Tests the d_test_options_new function.
  Tests the following:
...
)
{
    ...
    result = d_assert_standalone(
        ...
        "d_test_options_new should return non-NULL",
        _counter) && result;
    ...
    // test 5: zero flags
    config_zero = d_test_options_new(0);

    result = d_assert_standalone(
        config_zero != NULL,
        "new_zero_flags",
        "d_test_options_new(0) should return non-NULL",
        _counter) && result;
    ...
    if (config_zero)
    {
        ...
        d_test_options_free(config_zero);
    }
...
)
{
    ...
    copy = d_test_options_new_copy(original);

    // test 1: copy is non-NULL
    result = d_assert_standalone(
        copy != NULL,
        "new_copy_non_null",
        "d_test_options_new_copy should return non-NULL",
        _counter) && result;
    ...
    if (copy)
    {
        ...
        d_test_options_free(copy);
    }

    d_test_options_free(original);
...
)
{
    ...
    result = d_assert_standalone(
        ...
        "d_test_options_new_copy(NULL) should return NULL",
        _counter) && result;
...
d_tests_sa_config_free
  Tests the d_test_options_free function.
  Tests the following:
...
)
{
    ...
    result = d_assert_standalone(
        true,
        "free_null_safe",
        "d_test_options_free(NULL) should not crash",
        _counter) && result;
