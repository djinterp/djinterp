...
bool d_tests_sa_handler_empty_blocks(struct d_test_counter* _test_info)
{
    ...
    {struct d_test_handler*h=d_test_handler_new(NULL);
     if(h){struct d_test_block*b=d_test_block_new(10,NULL);
       if(b){bool r=d_test_handler_run_block(h,b,NULL);
         struct d_test_result res=d_test_handler_get_results(h);
         if(!d_assert_standalone((r==true&&res.blocks_run==1&&res.tests_run==0),
             "empty block","Empty block: succeeds, blocks=1, tests=0",_test_info))ok=false;
         d_test_block_free(b);}
       // Nested empty blocks
       struct d_test_block*o=d_test_block_new(5,NULL);struct d_test_block*in=d_test_block_new(3,NULL);
       if(o&&in){struct d_test_type*ity=d_test_type_new(D_TEST_TYPE_TEST_BLOCK,in);
         o->tests[0]=*ity;o->count=1;free(ity);
         d_test_handler_reset_results(h);
         d_test_handler_run_block(h,o,NULL);
         struct d_test_result r2=d_test_handler_get_results(h);
         if(!d_assert_standalone((r2.blocks_run==2&&r2.max_depth==2),
             "nested empty blocks","Nested empty: blocks=2, depth=2",_test_info))ok=false;
         d_test_block_free(o);}
