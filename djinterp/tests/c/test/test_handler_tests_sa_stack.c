/******************************************************************************
* djinterp [test]                                  test_handler_tests_sa_stack.c
*
*   Unit tests for test_handler stack operations:
*     d_test_handler_push_result, d_test_handler_pop_result,
*     d_test_handler_push_context, d_test_handler_pop_context
*
*
...
d_tests_sa_handler_result_stack
  ...
  Tests:
  - Push NULL result on valid handler (should fail)
  - Push valid result on handler with stack
  - Pop result retrieves pushed data
  - Multiple push/pop in LIFO order
  - Push on NULL handler (should fail)
  - Pop on NULL handler (should fail)
*/
bool
d_tests_sa_handler_result_stack
(
...
)
{
    ...
    // Test 1: Push/pop on NULL handler
    {
        ...
        if (!d_assert_standalone(
            push_result == false && pop_result == false,
    ...
    // Test 2: Push NULL result on valid handler
    {
    ...
    // Test 3: Push and pop a single result
    {
        struct d_test_handler* handler;
        ...
        if (handler && handler->result_stack)
        {
            ...
            push_ok = d_test_handler_push_result(handler, &push_data);
            pop_ok  = d_test_handler_pop_result(handler, &pop_data);
    ...
    // Test 4: Multiple push/pop in LIFO order
    {
        struct d_test_handler* handler;
        ...
        if (handler && handler->result_stack)
        {
            ...
            if (out.tests_run != 3) lifo_correct = false;

            d_test_handler_pop_result(handler, &out);
            if (out.tests_run != 2) lifo_correct = false;

            d_test_handler_pop_result(handler, &out);
            if (out.tests_run != 1) lifo_correct = false;
...
)
{
    ...
    // Test 3: Push and pop preserves fields
    {
        struct d_test_handler* handler;
        ...
        if (handler && handler->context_stack)
        {
            ...
            push_ok = d_test_handler_push_context(handler, &push_ctx);
            pop_ok  = d_test_handler_pop_context(handler, &pop_ctx);
    ...
    // Test 4: Pop from empty context stack
    {
        struct d_test_handler* handler;
    ...
    // Test 5: Auto-push during test execution with TRACK_STACK
    {
        struct d_test_handler* handler;
        ...
        if (handler && handler->context_stack)
        {
            ...
            if (test)
            {
                ...
                pop_ok = d_test_handler_pop_context(handler, &popped);

                if (!d_assert_standalone(
                    (pop_ok && popped.result == true),
...
d_tests_sa_handler_stack_no_alloc
  ...
  Tests:
  - Push result returns false when no result_stack
  - Pop result returns false when no result_stack
  - Push context returns false when no context_stack
  - Pop context returns false when no context_stack
  - Handler without stacks still functions for tests/blocks
*/
bool
d_tests_sa_handler_stack_no_alloc
(
