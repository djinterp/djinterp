...
)
{
    ...
    // test 5: at beginning
    const char* haystack_begin = "Testing 123";
    result = d_strcasestr(haystack_begin, "test");
    test_at_beginning = (result != NULL) && 
                       (result == haystack_begin);
...
)
{
    ...
    const char* multi_o_str = "multiple o's here";
    result = d_strchrnul(multi_o_str, 'o');
    test_first_occurrence = (result != NULL) && 
                           (result == strchr(multi_o_str, 'o'));
